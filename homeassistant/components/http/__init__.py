...
CONF_IP_BAN_EXCLUDED_NETWORKS: Final = "ip_ban_excluded_networks"
    ...
    vol.Schema(
        {
            ...
            vol.Optional(CONF_IP_BAN_EXCLUDED_NETWORKS, default=[]): vol.All(
                cv.ensure_list, [ip_network]
            ),
...
class ConfData(TypedDict, total=False):
    ...
    ip_ban_enabled: bool
    ip_ban_excluded_networks: list[IPv4Network | IPv6Network]
    ssl_profile: str
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    ip_ban_excluded_networks = conf.get(CONF_IP_BAN_EXCLUDED_NETWORKS) or []
    ...
    await server.async_initialize(
        ...
        ip_ban_excluded_networks=ip_ban_excluded_networks,
...
class HomeAssistantHTTP:
    ...
    async def async_initialize(
        ...
        *,
        ...
        if is_ban_enabled:
            setup_bans(self.hass, self.app, login_threshold, ip_ban_excluded_networks)
...
async def start_http_server_and_save_config(
    ...
    if CONF_IP_BAN_EXCLUDED_NETWORKS in conf:
        conf[CONF_IP_BAN_EXCLUDED_NETWORKS] = [
            str(cast(IPv4Network | IPv6Network, ip).network_address)
            for ip in conf[CONF_IP_BAN_EXCLUDED_NETWORKS]
        ]
